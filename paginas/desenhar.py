...
def show_desenho():
    ...
    while True:
        ...
        if hand:
            ...
            if (botao_salvar[0] < x < botao_salvar[0] + botao_raio * 2) and (botao_salvar[1] < y < botao_salvar[1] + botao_raio * 2):
                tempo_atual = time.time()
                if tempo_atual - ultimo_tempo > intervalo:
                    salvar_desenho(img_borda, pontos_desenhos_anteriores, pontos_atual)
                    ultimo_tempo = tempo_atual

            if (botao_apagar[0] < x < botao_apagar[0] + botao_raio * 2) and (botao_apagar[1] < y < botao_apagar[1] + botao_raio * 2):
                tempo_atual = time.time()
                if tempo_atual - ultimo_tempo > intervalo:
            ...
            if (botao_borracha[0] - botao_raio < x < botao_borracha[0] + botao_raio) and (botao_borracha[1] - botao_raio < y < botao_borracha[1] + botao_raio):
