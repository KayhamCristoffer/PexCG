...
def show_desenho():
    ...
    # Captura de vídeo da câmera com st.camera_input
    video_feed = st.camera_input("Capture uma imagem", key="camera_input")
    ...
    while True:
        if video_feed is not None:
            img = video_feed.read()  # Lê a imagem da câmera
            if img is None:
            ...
            # Atualiza o feed da câmera na interface do Streamlit
            st.image(cv2.cvtColor(img_borda, cv2.COLOR_BGR2RGB), channels="RGB")

        else:
            st.warning("Por favor, permita o acesso à câmera.")
...
def salvar_desenho(img, pontos_desenhos_anteriores, pontos_atual):
    ...
    # Adiciona o caminho da nova imagem na lista
    st.session_state.saved_images.append(caminho_completo)
    st.success("Desenho salvo com sucesso!")

# Chamando a função para mostrar a página de desenho
